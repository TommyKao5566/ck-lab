...
CKEDITOR.plugins.acertest = {
	requires: 'dialog',
	// jscs:disable maximumLineLength
	lang: 'af,ar,az,bg,bn,bs,ca,cs,cy,da,de,de-ch,el,en,en-au,en-ca,en-gb,eo,es,es-mx,et,eu,fa,fi,fo,fr,fr-ca,gl,gu,he,hi,hr,hu,id,is,it,ja,ka,km,ko,ku,lt,lv,mk,mn,ms,nb,nl,no,oc,pl,pt,pt-br,ro,ru,si,sk,sl,sq,sr,sr-latn,sv,th,tr,tt,ug,uk,vi,zh,zh-cn', // %REMOVE_LINE_CORE%
	// jscs:enable maximumLineLength
	init: function( editor ) {
		var cmd = new CKEDITOR.dialogCommand( 'acertest' );
		cmd.editorFocus = false;

		editor.addCommand( 'acertest', cmd );

		CKEDITOR.dialog.add( 'acertest', this.path + 'dialogs/acertest.js' );
		...
		editor.getColorFromDialog = function( callback, scope, options ) {
			...
			editor.execCommand( 'acertest' );

			if ( editor._.storedDialogs && editor._.storedDialogs.acertest ) {
				bindToDialog( editor._.storedDialogs.acertest );
			} else {
				CKEDITOR.on( 'dialogDefinition', function( e ) {
					if ( e.data.name != 'acertest' ) {
						return;
					}
...
CKEDITOR.plugins.add( 'acertest', CKEDITOR.plugins.acertest );
